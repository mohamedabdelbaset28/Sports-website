...
<html>
	<head>
		...
		<meta charset="utf-8">
		<meta name="description" content="Premier League Page">
		<style type="text/css">
	...
	<body>	
		...
		<h2 id="headtable1" class="headers">Premier League Table</h2>
		<?php
				...
				$query="SELECT * FROM `preimer league` ORDER BY `preimer league`.`pts` DESC";
			...
			<?php
				$query="SELECT * FROM `top goals_assists pl` ORDER BY `top goals_assists pl`.`goals` DESC";
				$result=mysqli_query($link,$query);
				$i=0;
				if ($result->num_rows > 0) {
			...
			<?php
				$query="SELECT * FROM `top goals_assists pl` ORDER BY `top goals_assists pl`.`assists` DESC";
				$result=mysqli_query($link,$query);
				$i=0;
				if ($result->num_rows > 0) {
